...
public class FileExplorer implements AutoCloseable {
	...
	public static void launch(JFrame parent, JTextComponent field, Directory d) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					@SuppressWarnings("resource")
					FileExplorer window = new FileExplorer(parent, field, d);
	 ...
	 */
	public FileExplorer(JFrame parent, JTextComponent field, Directory d) {
		parentFrame = parent;
		targetField = field;
		currentDirectory = d;
		selection = null;
	...
	private void initialize() {
		...
		toolBar.add(panel_3);
		FormLayout fl_panel_3 = new FormLayout(
				new ColumnSpec[] { ColumnSpec.decode("40px"), ColumnSpec.decode("pref:grow(2)"), },
				new RowSpec[] { RowSpec.decode("fill:20px"), });
		...
		panel_2.setLayout(
				new FormLayout(new ColumnSpec[] { ColumnSpec.decode("135px:grow"), ColumnSpec.decode("right:65px"), },
						new RowSpec[] { RowSpec.decode("25px"), }));
		...
		frame.getContentPane().setFocusTraversalPolicy(
				new FocusTraversalOnArray(new Component[] { panel, panel_1, toolBar, panel_3, pathField, btnParent,
						panel_2, btnSelection, selectionField, listView, scrollPaneTree, splitPane }));
